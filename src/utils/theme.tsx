import type { LinkProps } from '@mui/material/Link';
import type {
  Color,
  PaletteOptions,
  ThemeOptions,
  TypographyStyle,
  TypographyVariantsOptions,
} from '@mui/material/styles';
import { createTheme } from '@mui/material/styles';
import type { CSSProperties } from 'react';
import { forwardRef } from 'react';
import type { LinkProps as RouterLinkProps } from 'react-router-dom';
import { Link as RouterLink, useLocation } from 'react-router-dom';

declare module '@mui/material/styles' {
  interface TypographyVariants {
    mono: CSSProperties;
  }

  // allow configuration using `createTheme()`
  interface TypographyVariantsOptions {
    mono?: CSSProperties;
  }
}

// Update the Typography's variant prop options
declare module '@mui/material/Typography' {
  interface TypographyPropsVariantOverrides {
    mono: true;
  }
}

interface HaikuPalette extends PaletteOptions {
  indigo: Partial<Color>;
  deepPurple: Partial<Color>;
  amber: Partial<Color>;
  orange: Partial<Color>;
  pink: Partial<Color>;
  deepOrange: Partial<Color>;
  green: Partial<Color>;
  red: Partial<Color>;
  lightgreen: Partial<Color>;
  purple: Partial<Color>;
  lime: Partial<Color>;
  lightblue: Partial<Color>;
  yellow: Partial<Color>;
  cyan: Partial<Color>;
  teal: Partial<Color>;
  blue: Partial<Color>;
  blueGrey: Partial<Color>;
}

interface HaikuTypography extends TypographyVariantsOptions {
  mono: TypographyStyle;
}

interface HaikuTheme extends ThemeOptions {
  palette: HaikuPalette;
  typography: HaikuTypography;
}

const fontsDefault = [
  '-apple-system',
  'BlinkMacSystemFont',
  '"Segoe UI"',
  'Roboto',
  '"Helvetica Neue"',
  'Arial',
  'sans-serif',
  '"Apple Color Emoji"',
  '"Segoe UI Emoji"',
  '"Segoe UI Symbol"',
];

const fontsRajdhani = ['Rajdhani', ...fontsDefault].join(',');

const fontsOrbitron = ['Orbitron', ...fontsDefault].join(',');

const fontsCairo = ['Cairo', ...fontsDefault].join(',');

const fontsInconsolata = ['Inconsolata', ...fontsDefault].join(',');

const fontsEconomica = ['Economica', ...fontsDefault].join(',');

const fontsBebas = ['Bebas Neue', ...fontsDefault].join(',');

const fontsOxanium = ['Oxanium', ...fontsDefault].join(',');

const fontsOpenSans = ['Open Sans', ...fontsDefault].join(',');

const LinkBehavior = forwardRef<HTMLAnchorElement, Omit<RouterLinkProps, 'to'> & { href: RouterLinkProps['to'] }>(
  (props, ref) => {
    const { href, onClick, ...other } = props;
    // Map href (Material UI) -> to (react-router)

    const { search } = useLocation();

    const getPersistentHref = (destination: typeof href): string => {
      const params = new URLSearchParams(search);
      const utmKeys = ['utm_source', 'utm_medium', 'utm_campaign'];

      // Create a record of existing UTMs
      const utmParams = new URLSearchParams();
      utmKeys.forEach((key) => {
        const value = params.get(key);
        if (value) utmParams.set(key, value);
      });

      const utmString = utmParams.toString();
      if (!utmString) return destination?.toString() || '';

      // Append to string destination, preserving any hash fragment
      const destinationString = destination?.toString() || '';
      const [pathAndQuery, hash = ''] = destinationString.split('#', 2);
      const separator = pathAndQuery.includes('?') ? '&' : '?';
      const destinationWithUtm = `${pathAndQuery}${separator}${utmString}`;

      return hash ? `${destinationWithUtm}#${hash}` : destinationWithUtm;
    };

    const getDestinationHash = (destination: typeof href): string => {
      if (!destination) return '';

      if (typeof destination === 'string') {
        const hashIndex = destination.indexOf('#');
        return hashIndex >= 0 ? destination.slice(hashIndex) : '';
      }

      return destination.hash || '';
    };

    const destinationHash = getDestinationHash(href);

    const handleClick = (event: React.MouseEvent<HTMLAnchorElement>) => {
      onClick?.(event);
      if (event.defaultPrevented || !destinationHash) return;

      window.setTimeout(() => {
        const normalizedHash = destinationHash.startsWith('#') ? destinationHash : `#${destinationHash}`;
        const selectorHash = normalizedHash.trim();
        if (!selectorHash || selectorHash === '#') return;

        const elementId = selectorHash.slice(1);
        const decodedId = elementId ? decodeURIComponent(elementId) : '';
        const target =
          (decodedId ? document.getElementById(decodedId) : null) || document.querySelector<HTMLElement>(selectorHash);

        target?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      }, 120);
    };

    return <RouterLink ref={ref} to={getPersistentHref(href)} onClick={handleClick} {...other} />;
  },
);

const defaultPalette: HaikuTheme = {
  palette: {
    grey: {
      50: 'rgb(250, 250, 250)',
      100: 'rgb(245, 245, 245)',
      200: 'rgb(238, 238, 238)',
      300: 'rgb(224, 224, 224)',
      400: 'rgb(189, 189, 189)',
      500: 'rgb(158, 158, 158)',
      600: 'rgb(117, 117, 117)',
      700: 'rgb(97, 97, 97)',
      800: 'rgb(66, 66, 66)',
      900: 'rgb(33, 33, 33)',
      A100: 'rgb(213, 213, 213)',
      A200: 'rgb(170, 170, 170)',
      A400: 'rgb(97, 97, 97)',
      A700: 'rgb(48, 48, 48)',
    },
    indigo: {
      50: 'rgb(232, 234, 246)',
      100: 'rgb(197, 202, 233)',
      200: 'rgb(159, 168, 218)',
      300: 'rgb(121, 134, 203)',
      400: 'rgb(92, 107, 192)',
      500: 'rgb(63, 81, 181)',
      600: 'rgb(57, 73, 171)',
      700: 'rgb(48, 63, 159)',
      800: 'rgb(40, 53, 147)',
      900: 'rgb(26, 35, 126)',
      A100: 'rgb(140, 158, 255)',
      A200: 'rgb(83, 109, 254)',
      A400: 'rgb(61, 90, 254)',
      A700: 'rgb(48, 79, 254)',
    },
    deepPurple: {
      50: 'rgb(237, 231, 246)',
      100: 'rgb(209, 196, 233)',
      200: 'rgb(179, 157, 219)',
      300: 'rgb(149, 117, 205)',
      400: 'rgb(126, 87, 194)',
      500: 'rgb(103, 58, 183)',
      600: 'rgb(94, 53, 177)',
      700: 'rgb(81, 45, 168)',
      800: 'rgb(69, 39, 160)',
      900: 'rgb(49, 27, 146)',
      A100: 'rgb(179, 136, 255)',
      A200: 'rgb(124, 77, 255)',
      A400: 'rgb(101, 31, 255)',
      A700: 'rgb(98, 0, 234)',
    },
    amber: {
      50: 'rgb(255, 248, 225)',
      100: 'rgb(255, 236, 179)',
      200: 'rgb(255, 224, 130)',
      300: 'rgb(255, 213, 79)',
      400: 'rgb(255, 202, 40)',
      500: 'rgb(255, 193, 7)',
      600: 'rgb(255, 179, 0)',
      700: 'rgb(255, 160, 0)',
      800: 'rgb(255, 143, 0)',
      900: 'rgb(255, 111, 0)',
      A100: 'rgb(255, 229, 127)',
      A200: 'rgb(255, 215, 64)',
      A400: 'rgb(255, 196, 0)',
      A700: 'rgb(255, 171, 0)',
    },
    orange: {
      50: 'rgb(255, 243, 224)',
      100: 'rgb(255, 224, 178)',
      200: 'rgb(255, 204, 128)',
      300: 'rgb(255, 183, 77)',
      400: 'rgb(255, 167, 38)',
      500: 'rgb(255, 152, 0)',
      600: 'rgb(251, 140, 0)',
      700: 'rgb(245, 124, 0)',
      800: 'rgb(239, 108, 0)',
      900: 'rgb(230, 81, 0)',
      A100: 'rgb(255, 209, 128)',
      A200: 'rgb(255, 171, 64)',
      A400: 'rgb(255, 145, 0)',
      A700: 'rgb(255, 109, 0)',
    },
    pink: {
      50: 'rgb(252, 228, 236)',
      100: 'rgb(248, 187, 208)',
      200: 'rgb(244, 143, 177)',
      300: 'rgb(240, 98, 146)',
      400: 'rgb(236, 64, 122)',
      500: 'rgb(233, 30, 99)',
      600: 'rgb(216, 27, 96)',
      700: 'rgb(194, 24, 91)',
      800: 'rgb(173, 20, 87)',
      900: 'rgb(136, 14, 79)',
      A100: 'rgb(255, 128, 171)',
      A200: 'rgb(255, 64, 129)',
      A400: 'rgb(245, 0, 87)',
      A700: 'rgb(197, 17, 98)',
    },
    deepOrange: {
      50: 'rgb(251, 233, 231)',
      100: 'rgb(255, 204, 188)',
      200: 'rgb(255, 171, 145)',
      300: 'rgb(255, 138, 101)',
      400: 'rgb(255, 112, 67)',
      500: 'rgb(255, 87, 34)',
      600: 'rgb(244, 81, 30)',
      700: 'rgb(230, 74, 25)',
      800: 'rgb(216, 67, 21)',
      900: 'rgb(191, 54, 12)',
      A100: 'rgb(255, 158, 128)',
      A200: 'rgb(255, 110, 64)',
      A400: 'rgb(255, 61, 0)',
      A700: 'rgb(221, 44, 0)',
    },
    green: {
      50: 'rgb(232, 245, 233)',
      100: 'rgb(200, 230, 201)',
      200: 'rgb(165, 214, 167)',
      300: 'rgb(129, 199, 132)',
      400: 'rgb(102, 187, 106)',
      500: 'rgb(76, 175, 80)',
      600: 'rgb(67, 160, 71)',
      700: 'rgb(56, 142, 60)',
      800: 'rgb(46, 125, 50)',
      900: 'rgb(27, 94, 32)',
      A100: 'rgb(185, 246, 202)',
      A200: 'rgb(105, 240, 174)',
      A400: 'rgb(0, 230, 118)',
      A700: 'rgb(0, 200, 83)',
    },
    red: {
      50: 'rgb(254, 235, 238)',
      100: 'rgb(254, 205, 210)',
      200: 'rgb(239, 154, 154)',
      300: 'rgb(229, 115, 115)',
      400: 'rgb(239, 83, 80)',
      500: 'rgb(244, 67, 54)',
      600: 'rgb(229, 57, 53)',
      700: 'rgb(211, 47, 47)',
      800: 'rgb(198, 40, 40)',
      900: 'rgb(183, 28, 28)',
      A100: 'rgb(255, 138, 128)',
      A200: 'rgb(255, 82, 82)',
      A400: 'rgb(255, 23, 68)',
      A700: 'rgb(213, 0, 0)',
    },
    lightgreen: {
      50: 'rgb(241, 248, 233)',
      100: 'rgb(220, 237, 200)',
      200: 'rgb(197, 225, 165)',
      300: 'rgb(174, 213, 129)',
      400: 'rgb(139, 195, 74)',
      500: 'rgb(139, 195, 74)',
      600: 'rgb(124, 179, 66)',
      700: 'rgb(104, 159, 56)',
      800: 'rgb(85, 139, 47)',
      900: 'rgb(51, 105, 30)',
      A100: 'rgb(204, 255, 144)',
      A200: 'rgb(178, 255, 89)',
      A400: 'rgb(118, 255, 3)',
      A700: 'rgb(100, 221, 23)',
    },
    purple: {
      50: 'rgb(243, 229, 245)',
      100: 'rgb(225, 190, 231)',
      200: 'rgb(206, 147, 216)',
      300: 'rgb(186, 104, 200)',
      400: 'rgb(171, 71, 188)',
      500: 'rgb(156, 39, 176)',
      600: 'rgb(142, 36, 170)',
      700: 'rgb(123, 31, 162)',
      800: 'rgb(106, 27, 154)',
      900: 'rgb(74, 20, 140)',
      A100: 'rgb(234, 128, 252)',
      A200: 'rgb(224, 64, 251)',
      A400: 'rgb(213, 0, 249)',
      A700: 'rgb(170, 0, 255)',
    },
    lime: {
      50: 'rgb(249, 251, 231)',
      100: 'rgb(240, 244, 195)',
      200: 'rgb(230, 238, 156)',
      300: 'rgb(220, 231, 117)',
      400: 'rgb(212, 225, 87)',
      500: 'rgb(205, 220, 57)',
      600: 'rgb(192, 202, 51)',
      700: 'rgb(175, 180, 43)',
      800: 'rgb(158, 157, 36)',
      900: 'rgb(130, 119, 23)',
      A100: 'rgb(244, 255, 129)',
      A200: 'rgb(238, 255, 65)',
      A400: 'rgb(198, 255, 0)',
      A700: 'rgb(174, 234, 0)',
    },
    lightblue: {
      50: 'rgb(225, 245, 254)',
      100: 'rgb(179, 229, 252)',
      200: 'rgb(129, 212, 250)',
      300: 'rgb(79, 195, 247)',
      400: 'rgb(41, 182, 246)',
      500: 'rgb(3, 169, 244)',
      600: 'rgb(3, 155, 229)',
      700: 'rgb(2, 136, 209)',
      800: 'rgb(2, 119, 189)',
      900: 'rgb(1, 87, 155)',
      A100: 'rgb(128, 216, 255)',
      A200: 'rgb(64, 196, 255)',
      A400: 'rgb(0, 176, 255)',
      A700: 'rgb(0, 145, 234)',
    },
    yellow: {
      50: 'rgb(255, 253, 231)',
      100: 'rgb(255, 249, 196)',
      200: 'rgb(255, 245, 157)',
      300: 'rgb(255, 241, 118)',
      400: 'rgb(255, 238, 88)',
      500: 'rgb(255, 235, 59)',
      600: 'rgb(253, 216, 53)',
      700: 'rgb(251, 192, 45)',
      800: 'rgb(249, 168, 37)',
      900: 'rgb(245, 127, 23)',
      A100: 'rgb(255, 255, 141)',
      A200: 'rgb(255, 255, 0)',
      A400: 'rgb(255, 234, 0)',
      A700: 'rgb(255, 214, 0)',
    },
    cyan: {
      50: 'rgb(224, 247, 250)',
      100: 'rgb(178, 235, 242)',
      200: 'rgb(128, 222, 234)',
      300: 'rgb(77, 208, 225)',
      400: 'rgb(38, 198, 218)',
      500: 'rgb(0, 188, 212)',
      600: 'rgb(0, 172, 193)',
      700: 'rgb(0, 151, 167)',
      800: 'rgb(0, 131, 143)',
      900: 'rgb(0, 96, 100)',
      A100: 'rgb(132, 255, 255)',
      A200: 'rgb(24, 255, 255)',
      A400: 'rgb(0, 229, 255)',
      A700: 'rgb(0, 184, 212)',
    },
    teal: {
      50: 'rgb(224, 242, 241)',
      100: 'rgb(178, 223, 219)',
      200: 'rgb(128, 203, 196)',
      300: 'rgb(77, 182, 172)',
      400: 'rgb(38, 166, 154)',
      500: 'rgb(0, 150, 136)',
      600: 'rgb(0, 137, 123)',
      700: 'rgb(0, 121, 107)',
      800: 'rgb(0, 105, 92)',
      900: 'rgb(0, 77, 64)',
      A100: 'rgb(167, 255, 235)',
      A200: 'rgb(100, 255, 218)',
      A400: 'rgb(29, 233, 182)',
      A700: 'rgb(0, 191, 165)',
    },
    blue: {
      50: 'rgb(227, 242, 253)',
      100: 'rgb(187, 222, 251)',
      200: 'rgb(144, 202, 249)',
      300: 'rgb(25, 118, 210)',
      400: 'rgb(66, 165, 245)',
      500: 'rgb(33, 150, 243)',
      600: 'rgb(30, 136, 229)',
      700: 'rgb(0, 87, 178)',
      800: 'rgb(21, 101, 192)',
      900: 'rgb(13, 71, 161)',
      A100: 'rgb(130, 177, 255)',
      A200: 'rgb(68, 138, 255)',
      A400: 'rgb(41, 121, 255)',
      A700: 'rgb(41, 98, 255)',
    },
    blueGrey: {
      50: 'rgb(236, 239, 241)',
      100: 'rgb(207, 216, 220)',
      200: 'rgb(176, 190, 197)',
      300: 'rgb(144, 164, 174)',
      400: 'rgb(120, 144, 156)',
      500: 'rgb(96, 125, 139)',
      600: 'rgb(84, 110, 122)',
      700: 'rgb(69, 90, 100)',
      800: 'rgb(55, 71, 79)',
      900: 'rgb(38, 50, 56)',
    },
  },
  components: {
    MuiLink: {
      defaultProps: {
        component: LinkBehavior,
      } satisfies LinkProps,
    },
    MuiButtonBase: {
      defaultProps: {
        LinkComponent: LinkBehavior,
      },
    },
  },
  typography: {
    h1: {
      fontSize: 40,
      fontFamily: fontsOxanium,
      fontWeight: 600,
    },
    h2: {
      fontSize: 32,
      fontFamily: fontsRajdhani,
      fontWeight: 600,
    },
    h3: {
      fontSize: 24,
      fontFamily: fontsRajdhani,
      fontWeight: 500,
    },
    h4: {
      fontSize: 22,
      fontFamily: fontsRajdhani,
      fontWeight: 500,
    },
    h5: {
      fontSize: 20,
      fontFamily: fontsRajdhani,
      fontWeight: 500,
    },
    h6: {
      fontSize: 18,
      fontFamily: fontsRajdhani,
      fontWeight: 500,
    },
    body1: {
      fontSize: 16,
      fontFamily: fontsOpenSans,
      fontWeight: 400,
    },
    body2: {
      fontSize: 14,
      fontFamily: fontsEconomica,
      fontWeight: 400,
    },
    subtitle1: {
      fontSize: 16,
      fontFamily: fontsOrbitron,
      fontWeight: 500,
    },
    subtitle2: {
      fontSize: 14,
      fontFamily: fontsCairo,
      fontWeight: 500,
    },
    overline: {
      fontSize: 12,
      fontFamily: fontsOpenSans,
      fontWeight: 400,
    },
    caption: {
      fontSize: 12,
      fontFamily: fontsBebas,
      fontWeight: 400,
    },
    button: {
      fontFamily: fontsRajdhani,
      fontWeight: 600,
    },
    mono: {
      fontSize: 16,
      fontFamily: fontsInconsolata,
      fontWeight: 500,
    },
  },
};

const theme = createTheme({
  ...defaultPalette,
  cssVariables: true,
  palette: {
    ...defaultPalette.palette,
    action: {
      ...defaultPalette.palette.action,
      hoverOpacity: 0.08,
      selectedOpacity: 0.16,
      disabledOpacity: 0.12,
      focusOpacity: 0.12,
    },
  },
});

export { theme };
